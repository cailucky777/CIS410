    ...
    float h, int nsteps, float *output_locations)
{
    ...
    for(int i = 0; i <= nsteps; i++){
        //k1
        float vector_1[2] = {output_locations[2 * (i + 1) - 2], output_locations[2 * (i + 1) - 1]};
        float *k1 = new float[2];
        EvaluateVectorFieldAtLocation(vector_1, dims, X, Y, F, k1);
        //k2
        float vector_2[2] = {output_locations[2 * (i + 1) - 2] + (h/2 * k1[0]), output_locations[2 * (i + 1) - 1]+(h/2 * k1[1])};
        float *k2 = new float[2];
        EvaluateVectorFieldAtLocation(vector_2, dims, X, Y, F, k2);
        //k3
        float vector_3[2] = {output_locations[2 * (i + 1) - 2] + (h/2 * k2[0]), output_locations[2 * (i + 1) - 1]+(h/2 * k2[1])};
        float *k3 = new float[2];
        EvaluateVectorFieldAtLocation(vector_3, dims, X, Y, F, k3);
        //k4
        float vector_4[2] = {output_locations[2 * (i + 1) - 2] + (h * k3[0]), output_locations[2 * (i + 1) - 1]+(h * k3[1])};
        float *k4 = new float[2];
        EvaluateVectorFieldAtLocation(vector_4, dims, X, Y, F, k4);
        //pi+1 = pi+(1/6)*h*(k1+2k2+2k3+k4)
        output_locations[2*(i + 1)] = vector_1[0] + h*(k1[0]+2*k2[0]+2*k3[0]+k4[0])/6;
        output_locations[2*(i + 1) + 1] = vector_1[1] + h*(k1[1]+2*k2[1]+2*k3[1]+k4[1])/6;
